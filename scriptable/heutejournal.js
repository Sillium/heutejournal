const apiUrl = "https://hpst0yf89c.execute-api.eu-central-1.amazonaws.com/prod/json"
const DEBUG = false
...
async function createWidget() {
    ...
    const logoImg = rowLogo.addImage(logo)
    logoImg.imageSize = new Size(130, 30)
...
async function getLogo(forceDownload) {
    ...
    let scriptPath = module.filename
    let libraryDir = scriptPath.replace(fm.fileName(scriptPath, true), fm.fileName(scriptPath, false))
    let path = fm.joinPath(libraryDir, "heutejournal.png")
    if (fm.fileExists(path) && !forceDownload) {
        return fm.readImage(path)
    } else {
        const req = new Request("https://upload.wikimedia.org/wikipedia/commons/thumb/5/5b/ZDF_heute-journal_Logo_2021.svg/320px-ZDF_heute-journal_Logo_2021.svg.png")
